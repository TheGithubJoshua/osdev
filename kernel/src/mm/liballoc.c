...
#ifdef DEBUG
size_t l_allocated = 0;		//< The real amount of memory allocated.
size_t l_inuse = 0;			//< The amount of memory in use (malloc'ed).
#endif
 ...
 */
static inline int getexp( size_t size )
{
	if ( size < (1<<MINEXP) )
	{
		#ifdef DEBUG
...
static void* 	liballoc_memcpy(void* s1, const void* s2, size_t n)
{
  char *cdest;
  char *csrc;
  size_t *ldest = (size_t*)s1;
  size_t *lsrc  = (size_t*)s2;
...
{
	size_t remainder = tag->real_size - sizeof(struct boundary_tag) - tag->size;
...
void *malloc(size_t size)
{
	...
	liballoc_lock();
		...
		size_t remainder = tag->real_size - size - sizeof( struct boundary_tag ) * 2; // Support a new tag + remainder
	...
	liballoc_lock();
		...
		if ( (tag->split_left == NULL) && (tag->split_right == NULL) )
		{
			...
			if ( l_completePages[ index ] == MAXCOMPLETE )
			{
				...
				size_t pages = tag->real_size / l_pageSize;
