...
typedef struct FIS_DEV_BITS {
    uint8_t   fis_type;       // Always 0x27 for Set Device Bits FIS
    uint8_t   flags;          // Flags to set or clear for the device
    uint8_t   device;         // Specifies which device on the port
    uint8_t   reserved;       // Reserved, set to 0
    uint32_t  lba;            // Logical Block Address (may be used in some contexts)
    uint16_t  count;          // Count of blocks or operations
    uint16_t  reserved2;      // Additional reserved field
} FIS_DEV_BITS;
